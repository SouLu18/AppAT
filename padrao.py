...
class HomeScreen(ft.Container):
    def __init__(self, page: ft.Page, user, go=None):
        super().__init__()
        ...
        if go:
            maq = go[0] if len(go) > 0 else None
            model = go[1] if len(go) > 1 else None
            componente = go[3] if len(go) > 3 else None
            problem = go[4] if len(go) > 4 else None

            link = 'https://appat-5805e-default-rtdb.firebaseio.com/'
        
            if problem:
                requisicao = requests.get(f'{link}/Equipamentos/{maq}/{model}/Problemas/{componente}.json')
                dic_trouble = requisicao.json()
                descricao = dic_trouble[problem]['descrição']
                solucao = dic_trouble[problem]['solução']
                page.add(
                    ft.Container(
                        margin=ft.Margin(20, 0, 60, 0),
                        content=ft.Row(
                            [
                                ft.IconButton(ft.icons.ARROW_BACK_ROUNDED, on_click=lambda e: self.selected_maq(page, maq, model)),
                                ft.Row(
                                    [
                                        ft.TextButton(content=ft.Text(f'{maq}', size=22), on_click=lambda e: self.selected_maq(page, maq, model)),
                                        ft.TextButton(content=ft.Text(f'{model}', size=22), on_click=lambda e: self.selected_comp(page, maq, model, componente)),
                                        ft.TextButton(content=ft.Text(f'{componente}', size=22), on_click=lambda e: self.selected_comp(page, maq, model, componente)),
                                        ft.TextButton(content=ft.Text(f'{problem}', size=22), disabled=True),
                                    ],
                                ),
                                ft.Container()
                                ],
                                                    ...
                                                    ft.Container(
                                                        ft.Column(
                                                            controls=[
                                                                ...
                                                                ft.Text(f'{descricao}', size=18, max_lines=6, overflow='ellipsis'),
                                                                ft.Divider(),
                                                                ft.Row([ft.IconButton('open_in_new')])
                                                            ],
                                                            spacing=0
                                                        ),
                    ...
                    content=ft.Row(
                        [
                            ft.Container(
                                ...
                                ink=True, on_click=lambda e: self.Arotec(page), padding=ft.padding.all(10),
                                shadow=ft.BoxShadow(
                                            spread_radius=1,
                                            blur_radius=16,
    ...
    def equip_list(self, page, maquinas):
        ...
        page.add(
            ft.Container(
                ...
                content=ft.Row(
                    [
                        ft.IconButton(ft.icons.ARROW_BACK_ROUNDED, on_click=lambda e: self.Arotec(page)),
                        ft.Row(
                            [
                                ft.TextButton(content=ft.Text("Arotec", size=22), on_click=lambda e: HomeScreen(page, self.user)),
                                ft.TextButton(content=ft.Text("Máquinas", size=22), on_click=lambda e: self.Arotec(page)),
            ...
            ft.Container(
                ...
                content=ft.Row(
                    [
                        ft.ElevatedButton(icon=ft.icons.ADD, text='Adicionar Máquina', on_click=lambda e: self.new_model(page, maquinas))
                    ]
                ),
            ),
            ft.Container(
    ...
    def selected_comp(self, page, maq, model, comp):
        ...
        def scroll_to_item(index):
            exp_list.controls[index].content.bgcolor = "lightblue"  # Destacar o item
            exp_list.update()
            exp_list.scroll_to(index)
                        ...
                        [
                                                ...
                                                ft.Container(
                                                    ft.Column(
                                                        controls= [
                                                            ...
                                                            ft.Divider(),
                                                            ft.Row(
                                                                controls=[ft.IconButton('open_in_new'), ft.IconButton('edit')] if self.user == 'admin' else [ft.IconButton('open_in_new')]
                                                            )
                            ...
                            ft.Container(
                                ft.Row(
                                    ...
                                    alignment='spaceBetween'if solucao != 'Adicionar Soluções' else 'end'
                                ),
        ...
        # if pos:
        #     for i in range(len(exp_list.controls)):
        #         if exp_list.controls[i].content.controls[0].controls[0].value == pos:
        #             scroll_to_item(i)
                

    def add_list(self, page, maq, model):                    
        link = 'https://appat-5805e-default-rtdb.firebaseio.com/'
        requisicao = requests.get(f'{link}/Equipamentos/{maq}/{model}/Problemas.json')
        ...
        for id in dic_options:
            self.list_options.controls.append(
                ft.Row(
                    [
                        ft.Icon(ft.icons.ARROW_RIGHT),
                        ft.TextButton(content=ft.Text(f'{id}', size=20), on_click= lambda e: self.add_list(page, maq, model)),
                        ft.Container(expand=True),
                        ft.PopupMenuButton()
                    ]
                )
