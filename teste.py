...
class HomeScreen(ft.Container):
    def __init__(self, page: ft.Page, user, firebase):
        ...
        if self.user == 'developer':
            page.add(
                ft.FloatingActionButton(
                    ...
                    tooltip='Developer',
                    on_click=lambda e: developermode()
                )
            )
            page.update()
        ...
        def developermode():
            ...
            link_dev = 'https://appat-5805e-default-rtdb.firebaseio.com/DEV'
...
class Arotec(ft.Container):
    ...
    def equip_list(self, page, maquinas):
        ...
        for id in dict_mach:
            ...
            mach_row = ft.Row(
                [
                    ft.Icon(ft.Icons.ARROW_RIGHT),
                    ft.TextButton(content=ft.Text(f'{id}', size=25), on_click=lambda e, id=id: self.categories(page, maquinas, id)),
        ...
        def edit(id):
            def save(verify = True):
                ...
                normalized = re.sub(r'\W+', '', self.edit.value).upper()
                ...
                if normalized in norm_keys:
                    dlg.actions.append(
                        ft.Container(
                            ft.Column([ft.Icon(ft.Icons.WARNING),ft.Text('Modelo já existente')], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                            alignment=ft.alignment.center
                        )
                    )
                    page.update()
            ...
            self.edit = ft.TextField(label='Máquina', border_radius=16, on_submit=lambda e: save(verify=True), autofocus=True, value=f'{id}')
            ...
            dlg = ft.AlertDialog(
                actions=[
                    ft.Column(
                        [
                            self.edit,
                            ft.FilledButton(text="Salvar", on_click=lambda e: save(verify=True))
                        ],
                        spacing=8,
        ...
        def new_model(page, mach):
            def save(verify = True):
                dlg.actions.clear()
                normalized = re.sub(r'\W+', '', self.name.value).upper()
                norm_keys = {re.sub(r'\W+', '', item).upper(): item for item in dict_mach}
                ...
                elif score >= threshold and verify and match != 'CATEGORIAS' and match != 'categorias':
                    dlg.actions.append(
                        ft.Container(
                            ft.Column(
                                [
            ...
            self.name = ft.TextField(label='Nome da Máquina', border_radius=16, on_submit=lambda e: save(verify=True), autofocus=True,)
        ...
        def new_issue(error=None, title=None, comp_selec=None):
            def exit():
                ...
                def handle_click():
                    ...
                    new_issue(self.problem.content.value, self.title.value, self.comp.value)
                    ...
                    actions=[
                        ft.Column(
                            [
                                ...
                                ft.Row(
                                    [
                                        ft.FilledButton(text="Voltar", on_click=lambda e: new_issue(self.problem.content.value, self.title.value)),
    ...
    def issue_content(self, page, maq, model, issue, categ):
        ...
        try:
            ...
            self.cont_way.content.controls[1].controls[3].on_click = lambda e: self.categories(page, maq, model)
            self.cont_way.content.controls[1].controls[2].on_click = lambda e: self.equip_list(page, maq)
        except IndexError:
            self.cont_way.content.controls[1].controls[-1].disabled = False
            self.cont_way.content.controls[1].controls.append(
        ...
        def add_causa(issue, text=''):
            def exit():
                ...
                def handle_click():
                    page.close(close_dlg)
                    add_causa(issue, self.add.content.value)
                close_dlg = ft.AlertDialog(
                    content=ft.Text(
                        f'⚠️Atenção⚠️\nTodos as informações inseridas serão perdidas.\nDeseja fechar?',
                        text_align='center', 
            ...
            def save():
                if self.add.content.value == '':
                    ...
                    data = {'causa': self.add.content.value}
            ...
            self.add = ft.Container(
                ft.TextField(
                    label='Causa:', border_radius=16, on_submit=lambda e: save(), 
...
def main(page: ft.Page):
    ...
    try:
        ...
        page.add(
            ...
            ft.TextButton("Tentar novamente", on_click=lambda e: main(page))
        )
